...
- エクスプローラーのコンテキストメニュー「NeeViewで開く」を追加する設定追加。インストーラー版ではインストール時に自動設定されます。 (設定 > 全般 > エクスプローラーのコンテキストメニューに登録)
...
- マルチモニターでウィンドウ最大化時に隣のウィンドウにはみだずことがある現象修正
- Susieサーバ接続タイムアウトを30秒から1秒に変更し、10回リトライするようにした
